...
public class UsersHttpRequestTest {
    ...
    @Test
    public void userEndPointShouldReturnCollectionWithTwoUsers() throws Exception {
    ...
    public void userEndPointPostNewUserShouldReturnBadUserResponse() throws Exception {
        User user = User.builder().email("dummy@email.com").name("Dummy").
                password("aw2s0m").build();
        Map response = this.restTemplate.postForObject(BASE_URL + port +
                USERS_PATH,user, Map.class);
        assertThat(response).isNotNull();
        assertThat(response.get("errors")).isNotNull();
        Map errors = (Map) response.get("errors");
        assertThat(errors.get("password")).isNotNull();
        assertThat(errors.get("password")).isEqualTo("Password must be at least 8 characters long " +
                "and contain at least one number, one uppercase, one lowercase and one special character");
    }
